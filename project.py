...
def gdisconnect():
    ...
    if result['status'] == '200':
        ...
        #return render_template('publiccatalog.html', catalogs = showAllCat, items = latest)
...
def showCatalog():
    ...
    if 'username' not in login_session:
        return render_template('publiccatalog.html', catalog = showAllCat, items = latest)
    else:
...
def showItems(cat_id):
	...
	return render_template('showCatalogItems.html', catalogs=selectCat, allcat = showAllCat,  items = showCatitems)
...
def itemDetail(cat_id, item_id):
	...
	session = DBSession()
	selectCat = session.query(Category).filter_by(id=cat_id).one()
	itemDetail = session.query(Items).filter_by(category_id = selectCat.id).filter_by(id= item_id).one()
...
def editItem(cat_id, item_id):
    ...
    editedItem = session.query(Items).filter_by(id = item_id).filter_by(category_id = editedCat.id).one()
    if request.method == 'POST':
        ...
        return render_template('editItem.html', cat_id = editedCat, item_id = editedItem, categories = allcat)
...
def  deleteItem(cat_id, item_id):
	...
	deleteItem = session.query(Items).filter_by(id = item_id).filter_by(category_id = deleteCat.id).one()
	if request.method == 'POST':
...
def getJson():
	...
	return jsonify(Category= [j.serialize for j in getallCat], Items=[i.serialize for i in getallItem])
